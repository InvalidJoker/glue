package de.joker.glue.paper.event

import de.joker.glue.paper.PluginInstance
import de.joker.glue.paper.tasks.delayTask
 ...
 */
inline fun <reified T: Event> cancelEvent(autoRegistration: Boolean = true, noinline condition: EventFilter<T>? = null) = listener<T> {
    this.autoRegistration = autoRegistration
    handler { event, _ ->
        if (event !is Cancellable) return@handler
        if (condition?.invoke(event) != false) {
